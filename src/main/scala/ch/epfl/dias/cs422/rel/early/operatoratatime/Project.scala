...
class Project protected (
  ...
  var tuple = input.toIndexedSeq
  /**
    * @inheritdoc
    */
  def execute(): IndexedSeq[Column] = {
    if(!tuple.isEmpty){
      var active = tuple.last
      tuple = tuple.dropRight(1)
      tuple = tuple.transpose
      val projects = tuple.map(elem => evaluator(elem))
     return  projects.transpose :+ active
    }else{
      return tuple
