...
class LazyFetchFilterRule protected (config: RelRule.Config)
  ...
  override def onMatchHelper(call: RelOptRuleCall): RelNode = {
    ...
    val col_filter : LateColumnScan = call.rel(2)
    val column = col_filter.getColumn

    val bla : Filter =  call.rel(1)

    val class_ = classOf[LogicalFetch]
    val newCondition = RexUtil.shift(bla.getCondition, 3)
    var fetch = LogicalFetch.create(input,call.rel(3), column, None, class_)
    var filt = LogicalFilter.create(fetch, newCondition)
    return filt
